...
interface Transport{
	 ...
	 */
	command uint16_t write(socket_t fd);
	...
	command error_t setNodeSocket(socket_t fd);
 
	command socket_t getNodeSocket();
 
	command error_t incoming(TCP data_TCP);
	
	command error_t send(TCP ack_TCP);
}
